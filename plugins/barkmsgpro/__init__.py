...
class BarkMsgPro(_PluginBase):
    ...
    # 作者主页
    author_url = "https://github.com/loongcheung"
    # 插件配置项ID前缀
    plugin_config_prefix = "barkmsgpro_"
    # 加载顺序
    plugin_order = 27
    # 可使用的用户级别
    auth_level = 1
    ...
    def init_plugin(self, config: dict = None):
        if config:
            ...
            self._includes = config.get("includes")
    ...
    def send(self, event: Event):
        ...
        includes_result = self.includes_keyword(path=title, keywords=self._includes)
        logger.info(includes_result.get("message", ""))
        
        if not includes_result.get("ret", False):
            return
